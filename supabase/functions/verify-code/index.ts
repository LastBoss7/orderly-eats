...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Try to get user from auth.users - may or may not exist depending on email confirmation
    const { data: userData, error: userError } = await supabaseAdmin.auth.admin.getUserById(userId)
    
    let user = userData?.user
    
    // If user exists, proceed with setup
    if (user) {
      console.log(`Found user ${userId}, checking for pending setup`)
      ...
      if (!existingProfile && user.user_metadata?.pending_setup) {
        ...
        console.log(`Restaurant and profile created successfully for user ${userId}`)
      ...
      // User doesn't exist in auth.users yet - this happens when auto_confirm is disabled
      // In this case, we've verified the token but can't confirm the user yet
      // The user needs to be fully created first
      console.log(`User ${userId} not found in auth.users - token verified but user not confirmed yet`)
      console.log('User error:', userError)
      
      // Still return success since the code was valid
      // The actual user confirmation will happen when they complete the signup flow
    }
