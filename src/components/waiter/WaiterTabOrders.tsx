...
interface WaiterTabOrdersProps {
  tab: Tab;
  onBack: () => void;
  onTabClosed: () => void;
...
export function WaiterTabOrders({ tab, onBack, onTabClosed, restaurantId: propRestaurantId }: WaiterTabOrdersProps) {
  ...
  }, [restaurant?.id, tab.id, fetchOrders]);
      ...
      <div className="min-h-screen bg-[#f5f5f5] flex flex-col">
        <header className="sticky top-0 bg-[#1e3a5f] text-white p-4 z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-bold">Fechar Comanda #{tab.number}</h1>
      ...
      <header className="sticky top-0 bg-[#1e3a5f] text-white p-4 z-10">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-bold">Comanda #{tab.number}</h1>
              {tab.customer_name && (
          ...
          <Badge 
            className={`${
              tab.status === 'occupied' ? 'bg-red-500' : 
              tab.status === 'closing' ? 'bg-yellow-500' : 'bg-green-500'
            } text-white border-0`}
          >
            {tab.status === 'occupied' ? 'Ocupada' : tab.status === 'closing' ? 'Fechando' : 'Livre'}
          </Badge>
        </div>
      </header>
          ...
          <div className="p-4 space-y-3">
            ...
            <div className="bg-gradient-to-br from-[#1e3a5f] to-[#2d5a87] rounded-xl p-4 text-white">
              <div className="flex justify-between items-center">
                <span className="text-lg">Total da Comanda</span>
